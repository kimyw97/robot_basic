...
const osMessageQueueAttr_t myQueue_attributes = { .name = "myQueue" };
/* USER CODE BEGIN PV */
...
int main(void) {
	...
	/* Create the queue(s) */
	/* creation of myQueue */
	myQueueHandle = osMessageQueueNew(16, sizeof(uint16_t),
			&myQueue_attributes);
	...
	defaultTaskHandle = osThreadNew(Producer_Task, NULL,
			&defaultTask_attributes);

	/* creation of myTask02 */
	myTask02Handle = osThreadNew(Consumer_Task, NULL, &myTask02_attributes);
...
void Producer_Task(void *argument) {
	...
	/* Infinite loop */
	for (;;) {
		osMessageQueuePut(myQueueHandle, &count, 0, osWaitForever);
		count++;
		osDelay(1000);
	}
	/* USER CODE END 5 */
...
void Consumer_Task(void *argument) {
	...
	/* Infinite loop */
	for (;;) {
		osMessageQueueGet(myQueueHandle, &received, NULL, osWaitForever);
		if (received % 2 == 0) {
